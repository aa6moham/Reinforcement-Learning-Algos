...
#---------------------#start of your code#---------------------#
  ...
  def getPolicy(self, state):
    ...
    "*** YOUR CODE HERE ***"
    best_action = possibleActions[np.argmax([self._qValues[state][action] for action in possibleActions])]
    return best_action
  ...
  def getAction(self, state):
    ...
    probs = np.ones(len(possibleActions)) * epsilon / len(possibleActions)
    best_action = self.getPolicy(state)
    probs[best_action] += 1.0 - epsilon
    selected_action = np.random.choice(possibleActions,p=probs)
  ...
  def update(self, state, action, next_state, reward):
    ...
    "*** YOUR CODE HERE ***"    
    reference_qvalue = reward + gamma * self.getQValue(next_state,self.getPolicy(next_state))
